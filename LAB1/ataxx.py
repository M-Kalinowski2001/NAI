"""
Gra Ataxx, zasady: https://en.wikipedia.org/wiki/Ataxx
Autorzy: Aleksander Misztal s22019, Michał Kalinowski s22464
Aby uruchomić grę należy odpalić IDE, i w odpowiedniej ścieżce wykonać komandę python ataxx.py
Poruszanie następuje po kordynatach wpisywanych po spacji między cyframi
...
class AtaxxGame:
    ...
    def display_board(self):
        ...
        """ Display column numbers as coordinates """
        col_numbers = '   ' + ' '.join(str(i) for i in range(self.BOARD_SIZE))
        print(col_numbers)

        """ Display row numbers as coordinates """
        for i, row in enumerate(self.board):
            row_str = f"{i} | " + ' '.join(row) + f" | {i}"
            print(row_str)
